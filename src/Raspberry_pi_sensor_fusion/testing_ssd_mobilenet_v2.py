...
#Naming the window and opening the video stream
cv2.namedWindow("preview")
vc = cv2.VideoCapture(0)

# try to get the first frame
if vc.isOpened(): 
    is_camera_connected, frame = vc.read()
...
while is_camera_connected:
    ...
    #inserting image into model and getting output
    model.setInput(cv2.dnn.blobFromImage(frame, size=(200, 200), swapRB=True))
    output = model.forward()
    
    for detection in output[0, 0, :, :]:
        confidence = detection[2]
        if confidence > .5:
            #putting out text and boxes that corresponds to the output
    ...
    #show the image and getting next one
